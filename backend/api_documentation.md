...
Обновление информации о текущем пользователе. Администраторы могут обновлять все поля, обычные пользователи — только ФИО, телефон и email.
...
- `username` (необязательный, string, только для админа): Имя пользователя (мин. 3 символа)
- `full_name` (необязательный, string): Полное имя
- `phone` (необязательный, string): Телефон
- `email` (необязательный, string): Email
- `is_active` (необязательный, boolean, только для админа): Статус активности
