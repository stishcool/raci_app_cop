...
class UserCreateSchema(Schema):
    ...
    positions = fields.List(fields.Int(), allow_none=True)  # Новый: список ID должностей
...
class UserUpdateSchema(Schema):
    ...
    positions = fields.List(fields.Int(), allow_none=True)  # Новый
...
def update_user(user_id):
    ...
    try:
        ...
        # Обновление должностей: удалить старые, добавить новые
...
@jwt_required(optional=True)  # ← КЛЮЧЕВОЕ: optional=True!
def get_positions():
    try:
        # identity может быть None — это нормально
        identity = get_jwt_identity()
        
        # Если нужно — можешь проверить, админ ли пользователь
        # current_user_id = int(identity) if identity else None
        # is_admin = is_admin_check(current_user_id) if current_user_id else False
        ...
        import traceback
        traceback.print_exc()  # ← Чтобы видеть полный traceback в консоли
...
def create_position():
    try:
        ...
        # Убрали is_custom — его нет в модели!
        position = Position(
            title=title,
            created_at=datetime.utcnow()
        )
