...
def get_project(project_id):
    ...
    return jsonify({
        ...
        'members': [m.user_id for m in project.members]  # Массив ID для фронта
    })
...
def delete_project_stage(project_id, stage_id):
    ...
    db.session.commit()
    return jsonify({'message': 'Этап удален'})
...
def get_project_members(project_id):
    try:
        ...
        # === ЗАМЕНА has_project_access НА ПРЯМУЮ ПРОВЕРКУ ===
        # Пользователь должен быть либо в участниках проекта, либо админом
        ...
        for member in members:
            ...
            user_positions = (
                db.session.query(Position.title)
            ...
            members_data.append({
                ...
                "email": user.email,
                "phone": user.phone,
                "positions": positions_list  # ← Теперь точно будет!
            })
        ...
        return jsonify({"error": "Ошибка загрузки участников", "details": str(e)}), 500
